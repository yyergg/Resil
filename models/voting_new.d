...
mode client_wait (true) { 
    when ?corr (true) may count_corr++1;
    when ?inco (true) may count_inco++1;
    when !u (count_corr > (CProc/2))
...
mode server_op (true) {
    when !u !inco (count_fault > 0)
        may count_fault--1; count_fault_to_reply++1;
    when !c !corr (count_run > 0)
        may count_run--1; count_run_to_reply++1;
